...
public class AppStateManager implements Application.ActivityLifecycleCallbacks {
    ...
    public void onActivityResumed(Activity activity) {
        ...
        if (wasBackground) {
            ExoticsLogger.d(TAG, "went foreground");
            for (Listener l : listeners) {
                try {
                    l.onBecameForeground();
                } catch (Exception exc) {
                    ExoticsLogger.e(TAG, "Listener threw exception!"+exc.getMessage());
                }
            }
        } else {
            ExoticsLogger.d(TAG, "still foreground");
        }
    }
        ...
        handler.postDelayed(check = new Runnable() {
            ...
            public void run() {
                if (foreground && paused) {
                    foreground = false;
                    ExoticsLogger.d(TAG, "went background");
                    for (Listener l : listeners) {
                        try {
