package com.wan.exotics.ui.widgets;
...
public class RoundedImageView extends DynamicHeightImageView {
    ...
    private Drawable resolveResource() {
        ...
        if (mResource != 0) {
            try {
                ...
                ExoticsLogger.w(TAG, "Unable to find resource: " + mResource);
                // Don't try again.
                mResource = 0;
            }
        }
     ...
     * @return the radius.
     */
    public float getCornerRadius(@RoundedImageCorner int corner) {
     ...
     */
    public void setCornerRadiusDimen(@RoundedImageCorner int corner, @DimenRes int resId) {
     ...
     */
    public void setCornerRadius(@RoundedImageCorner int corner, float radius) {
    ...
    public void setCornerRadius(float topLeft, float topRight, float bottomLeft, float bottomRight) {
        if (mCornerRadii[RoundedImageCorner.TOP_LEFT] == topLeft
                && mCornerRadii[RoundedImageCorner.TOP_RIGHT] == topRight
                && mCornerRadii[RoundedImageCorner.BOTTOM_RIGHT] == bottomRight
                && mCornerRadii[RoundedImageCorner.BOTTOM_LEFT] == bottomLeft) {
            return;
        }

        mCornerRadii[RoundedImageCorner.TOP_LEFT] = topLeft;
        mCornerRadii[RoundedImageCorner.TOP_RIGHT] = topRight;
        mCornerRadii[RoundedImageCorner.BOTTOM_LEFT] = bottomLeft;
        mCornerRadii[RoundedImageCorner.BOTTOM_RIGHT] = bottomRight;
